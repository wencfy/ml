...
class AffineNode(object):
    ...
    def backward(self):
        d_W = np.outer(self.d_out, self.x.out).reshape(self.W.d_out.shape)
...
class SoftmaxNode(object):
    """ Node applying the softmax function to an array """
    def __init__(self, a, node_name):
        """ 
        Parameters:
        a: node for which a.out is a numpy array
    ...
    def forward(self):
        e_a = np.exp(self.a.out - np.max(self.a.out))  # for numerical stability
        self.out = e_a / np.sum(e_a, axis=0, keepdims=True)
        self.d_out = np.zeros(self.out.shape)
        return self.out

    def backward(self):
        jacobian_matrix = np.diag(self.out) - np.outer(self.out, self.out)
        self.a.d_out += np.dot(jacobian_matrix, self.d_out)
        return self.d_out
...
class NLLNode(object):
    """ Node computing the negative log-likelihood loss """
    def __init__(self, y, t, node_name):
        """
        Parameters:
        y: node for which y.out is a numpy array representing the predicted probabilities
        t: node for which t.out is a numpy array representing the true labels (one-hot encoded)
        node_name: node's name (a string)
        """
        self.node_name = node_name
        self.y = y
    ...
    def forward(self):
        ...
        eps = 1e-15
        self.out = -np.sum(np.log(self.y.out + eps)[self.t.out])
        self.d_out = np.zeros(self.out.shape)
        return self.out

    def backward(self):
        d_y = np.zeros(self.y.d_out.shape)
        d_y[self.t.out] = -1 / self.y.out[self.t.out]
        self.y.d_out += d_y
        return self.d_out
