...
class MulticlassClassifier(BaseEstimator, RegressorMixin):
    ...
    def __init__(self, num_hidden_units=10, step_size=.005, init_param_scale=0.01, max_num_epochs = 1000, num_class=3):
        ...
        self.affine = nodes.AffineNode(self.W1, self.x, self.b1, "affine")
        self.tanh = nodes.TanhNode(self.affine, "hidden tanh")
        self.z = nodes.AffineNode(self.W2, self.tanh, self.b2, "z")
        self.prediction = nodes.SoftmaxNode(self.z, "soft max")

        self.objective = nodes.NLLNode(self.prediction, self.y, "NLL")
